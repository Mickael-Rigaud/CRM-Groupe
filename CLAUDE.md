...
- `supabase/*.sql` — scripts à exécuter **à la main** dans Supabase > SQL Editor, dans l'ordre : `schema.sql` → `modules-lot2.sql` → `lot3-documents-prets.sql` → `lot4-gestion-locative.sql` → `lot5-btp.sql` (fiches DTU et mails types) → … → `lot11-stats-structures.sql`. Tout nouveau changement de schéma = nouveau fichier `supabase/lotN-….sql` idempotent (`if not exists`, `drop policy if exists`…), jamais de modification des anciens.
